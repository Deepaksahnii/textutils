...
export default function TextForm(props) {
  ...
  <textarea className="form-control" value={text} onChange={handleOnChange} id="exampleFormControlTextarea1" rows="8" placeholder="Enter text here..." style={{backgroundColor: props.mode==='dark'? '#748aa8': 'white', color:props.mode=== 'dark'?'white': 'black'}}></textarea>
          ...
          <button disabled= {text.length===0} className="btn btn-primary mx-1" onClick={handleLowClick}>Convert to lower Case</button>
          <button disabled= {text.length===0} className="btn btn-primary mx-1" onClick={handleExtraClick}>Remove extra spaces</button>
          ...
          <button disabled= {text.length===0} className="btn btn-primary mx-1" onClick={handleEmail}>Extract emails</button>
      </div>
